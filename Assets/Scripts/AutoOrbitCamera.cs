...
public class AutoOrbitCamera : MonoBehaviour
{
    ...
    // 6: follow target random position (new spawned dream objects)

    private Vector3 targetRandomPosition; // New variable to store the target random position
    ...
    public void SetNewRandomPosition(Vector3 newPosition)
    {
        targetRandomPosition = newPosition;
    }
    ...
    private void PivotTransformations()
    {
        ...
        // Use the target random position for camera movement
        if (transformPivot.position != targetRandomPosition)
        {
            transformPivot.position = Vector3.Lerp(transformPivot.position, targetRandomPosition, Time.deltaTime * OrbitDampening);
        }
